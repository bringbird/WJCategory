...
/// 矩形裁剪框的颜色
#define RectangularColor [UIColor whiteColor]
...
@end
@interface WJImageCropperViewController ()
...
@end
@implementation WJImageCropperViewController
...
- (void)initView {
    ...
    self.ratioView.layer.borderColor = RectangularColor.CGColor;
...
- (void)cancel:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(WJImageCropperDelegate)]) {
...
@interface WJImagePicker ()<UINavigationControllerDelegate, UIImagePickerControllerDelegate,WJImageCropperDelegate>{
    double _scale;
}
@property (nonatomic, strong) UIImagePickerController *imagePickerController;
@property (nonatomic, strong) WJImageCropperViewController *imageCropperController;

@end

@implementation WJImagePicker

+ (instancetype)sharedInstance {
    static dispatch_once_t once;
    static WJImagePicker *sharedInstance = nil;
    dispatch_once(&once, ^{
        sharedInstance = [[WJImagePicker alloc] init];
    });
...
- (UIImagePickerController *)imagePickerController {
    if (!_imagePickerController) {
        ...
        _imagePickerController.navigationBar.barTintColor = RectangularColor;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    self.imageCropperController = [[WJImageCropperViewController alloc] initWithImage:image cropFrame:CGRectMake(0, (screenSize().height - screenSize().width * _scale) / 2, screenSize().width, screenSize().width * _scale) limitScaleRatio:5];
