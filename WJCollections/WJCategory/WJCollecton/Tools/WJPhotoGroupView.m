...
- (id)copyWithZone:(NSZone *)zone {
    WJPhotoGroupItem *item = [self.class new];
    return item;
}
@end
...
@interface WJPhotoGroupCell : UIScrollView <UIScrollViewDelegate>
...
@property (nonatomic, strong) WJPhotoGroupItem *item;
...
- (void)setItem:(WJPhotoGroupItem *)item {
...
@interface WJPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation WJPhotoGroupView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((WJPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
...
#pragma clang diagnostic pop
    ...
    WJPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    WJPhotoGroupItem *item = _groupItems[self.currentPage];
...
#pragma clang diagnostic pop
    ...
    WJPhotoGroupCell *cell = [self cellForPage:currentPage];
    WJPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(WJPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            WJPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                WJPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (WJPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (WJPhotoGroupCell *)dequeueReusableCell {
    WJPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (WJPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (WJPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    WJPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
...
#pragma clang diagnostic pop
                ...
                duration *= 0.8;
                duration = WJ_CLAMP(duration, 0.05, 0.3);
