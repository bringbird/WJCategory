...
- (instancetype)initPrivate {
    if (self = [super init]) {
        self.username = @"";
        self.password = @"";
        self.progressAction = ^(NSInteger totalSize, NSInteger finishedSize){};
        self.successAction = ^(Class resultClass, id result){};
        self.failAction = ^(NSError *error){};
...
- (void)setServerURL:(NSURL *)serverURL {
    if (serverURL.absoluteString.isValidateFTPURLString) {
        ...
        NSLog(@"WJFTPClient: The serverURL is illegal!");
    }
}
...
- (void)setLocalFileURL:(NSURL *)localFileURL {
    if (localFileURL.absoluteString.isValidateFileURLString) {
        ...
        NSLog(@"WJFTPClient: The localFileURL is illegal!");
    }
}
...
- (NSError *)errorWithCFError:(CFStreamError)cfError {
    return [NSError errorWithDomain:[NSString stringWithFormat:@"%ld",cfError.domain] code:(NSInteger)cfError.error userInfo:@{@"errorMessage":[self errorMessageOfCode:(NSInteger)cfError.error]}];
}

@end

@interface WJResourceListFTPClient : WJFTPClient

@property (nonatomic,strong) NSMutableData *listData;

@end

@implementation WJResourceListFTPClient
...
- (BOOL)start {
    ...
    Boolean supportsAsynchronousNotification = CFReadStreamSetClient(self.readStream, kCFStreamEventNone| kCFStreamEventOpenCompleted| kCFStreamEventHasBytesAvailable| kCFStreamEventCanAcceptBytes| kCFStreamEventErrorOccurred| kCFStreamEventEndEncountered, resourceListReadStreamClientCallBack, &_streamClientContext);
...
void resourceListReadStreamClientCallBack(CFReadStreamRef stream, CFStreamEventType type, void *clientCallBackInfo) {
    WJResourceListFTPClient * request = (__bridge WJResourceListFTPClient *)clientCallBackInfo;
    switch (type) {
        case kCFStreamEventNone: break;
        case kCFStreamEventOpenCompleted: break;
        case kCFStreamEventCanAcceptBytes: break;
        case kCFStreamEventErrorOccurred: {
            CFStreamError error = CFReadStreamGetError(stream);
            request.failAction([request errorWithCFError:error]);
            [request stop];
        }
            break;
        ...
        case kCFStreamEventHasBytesAvailable: {
            ...
            if (bytesRead > 0) {
                ...
                request.progressAction(0, (NSInteger)request.listData.length);
            } else if (bytesRead == 0) {
                ...
                do {
                    CFIndex bytesParsed = CFFTPCreateParsedResourceListing(kCFAllocatorDefault, &((const uint8_t *)request.listData.bytes)[totalBytesParsed], request.listData.length - totalBytesParsed, &parsedDictionary);
                    if (bytesParsed > 0) {
                        ...
                        request.progressAction(0, (NSInteger)totalBytesParsed);
...
@interface WJDownloadFTPClient : WJFTPClient

@end

@implementation WJDownloadFTPClient
...
- (BOOL)start {
    ...
    Boolean supportsAsynchronousNotification = CFReadStreamSetClient(self.readStream, kCFStreamEventNone| kCFStreamEventOpenCompleted| kCFStreamEventHasBytesAvailable| kCFStreamEventCanAcceptBytes| kCFStreamEventErrorOccurred| kCFStreamEventEndEncountered, downloadReadStreamClientCallBack, &_streamClientContext);
...
void downloadReadStreamClientCallBack(CFReadStreamRef stream, CFStreamEventType type, void *clientCallBackInfo) {
    WJDownloadFTPClient *request = (__bridge WJDownloadFTPClient *)clientCallBackInfo;
    switch (type) {
        case kCFStreamEventNone: break;
        case kCFStreamEventCanAcceptBytes: break;
            ...
            if (bytesRead > 0) {
                ...
                do {
                    ...
                    if (bytesWritten > 0) {
                        ...
                        request.progressAction(request.totalSize, request.finishedSize);
...
@interface WJUploadFTPClient : WJFTPClient

@end

@implementation WJUploadFTPClient
...
- (BOOL)start {
    ...
    Boolean supportsAsynchronousNotification = CFWriteStreamSetClient(self.writeStream, kCFStreamEventNone| kCFStreamEventOpenCompleted| kCFStreamEventHasBytesAvailable| kCFStreamEventCanAcceptBytes| kCFStreamEventErrorOccurred| kCFStreamEventEndEncountered, uploadWriteStreamClientCallBack, &_streamClientContext);
...
void uploadWriteStreamClientCallBack(CFWriteStreamRef stream, CFStreamEventType type, void *clientCallBackInfo) {
    WJUploadFTPClient * request = (__bridge WJUploadFTPClient *)clientCallBackInfo;
    switch (type) {
        case kCFStreamEventNone: break;
        case kCFStreamEventOpenCompleted: break;
        case kCFStreamEventHasBytesAvailable: break;
...
@interface WJCreateResourceFTPClient : WJFTPClient

@end

@implementation WJCreateResourceFTPClient
...
- (BOOL)start {
    ...
    CFWriteStreamSetProperty(self.writeStream, kCFStreamPropertyFTPUserName, (__bridge CFTypeRef)self.username);
    CFWriteStreamSetProperty(self.writeStream, kCFStreamPropertyFTPPassword, (__bridge CFTypeRef)self.password);
    Boolean supportsAsynchronousNotification = CFWriteStreamSetClient(self.writeStream, kCFStreamEventNone| kCFStreamEventOpenCompleted| kCFStreamEventHasBytesAvailable| kCFStreamEventCanAcceptBytes| kCFStreamEventErrorOccurred| kCFStreamEventEndEncountered, createResourceWriteStreamClientCallBack, &_streamClientContext);
...
void createResourceWriteStreamClientCallBack(CFWriteStreamRef stream, CFStreamEventType type, void *clientCallBackInfo) {
    WJCreateResourceFTPClient * request = (__bridge WJCreateResourceFTPClient *)clientCallBackInfo;
    switch (type) {
        case kCFStreamEventNone: break;
        case kCFStreamEventOpenCompleted: break;
        case kCFStreamEventHasBytesAvailable: break;
...
@interface WJDestoryResourceFTPClient : WJFTPClient

@end

@implementation WJDestoryResourceFTPClient
...
#pragma clang diagnostic pop
    if (destroyResourceSuccess) {
        ...
        self.failAction([NSError errorWithDomain:@"0" code:(NSInteger)errorCode userInfo:@{@"errprMessage":@"Unknown"}]);
        return NO;
    }
}
...
@implementation WJFTPClient (WJ)

+ (WJFTPClient *)resourceListRequest {
    return [[WJResourceListFTPClient alloc]initPrivate];
}

+ (WJFTPClient *)downloadRequest {
    return [[WJDownloadFTPClient alloc]initPrivate];
}

+ (WJFTPClient *)uploadRequest {
    return [[WJUploadFTPClient alloc]initPrivate];
}

+ (WJFTPClient *)createResourceRequest {
    return [[WJCreateResourceFTPClient alloc]initPrivate];
}

+ (WJFTPClient *)destoryResourceRequest {
    return [[WJDestoryResourceFTPClient alloc]initPrivate];
}
