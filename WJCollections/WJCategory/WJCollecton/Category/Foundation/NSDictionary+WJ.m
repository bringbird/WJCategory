...
- (NSString *)stringByTrimWithStr:(NSString *)str {
    NSCharacterSet *set = [NSCharacterSet whitespaceAndNewlineCharacterSet];
    return [str stringByTrimmingCharactersInSet:set];
}

- (void)textEnd {
    _text = [self stringByTrimWithStr:_text].mutableCopy;
    if (_text.length) {
...
+ (NSDictionary *)dictionaryWithXML:(id)xml {
    XMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[XMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[XMLDictionaryParser alloc] initWithData:xml];
    }
